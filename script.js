$(function() {
    ...
    $(document).on('keydown', function(e) {
        ...
        if (key === 37 && move_left === false && game_over === false && game_pause === false) {
            move_left = requestAnimationFrame(left);
        } else if (key === 39 && move_right === false && game_over === false && game_pause === false) {
            move_right = requestAnimationFrame(right);
        } else if (key === 65 && move_left1 === false && game_over === false && game_pause === false) {
            move_left1 = requestAnimationFrame(left1);
        } else if (key === 83 && move_right1 === false && game_over === false && game_pause === false) {
            move_right1 = requestAnimationFrame(right1);
        }
    });
    ...
    $(document).on('keyup', function(e) {
        ...
        if (key === 37 && game_over === false && game_pause === false) {
            ...
            move_left = false;
        } else if (key === 39 && game_over === false && game_pause === false) {
            cancelAnimationFrame(move_right);
            move_right = false;
        } else if (key === 65 && game_over === false && game_pause === false) {
            cancelAnimationFrame(move_left1);
            move_left1 = false;
        } else if (key === 83 && game_over === false && game_pause === false) {
    ...
    function stop_the_game() {
        ...
        cancelAnimationFrame(move_left1);
        game_over = true;
        winner.html('<br><span id="won">Player ' + who_won + ' won</span><br><br><button id="continue_btn">Continue</button><br><br><button id="restart_btn">Restart</button>').show();
        $('#continue_btn').focus();
    }
